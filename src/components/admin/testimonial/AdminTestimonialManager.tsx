...
export default function AdminTestimonialManager({
  ...
  const filteredItems = items.filter((testimonial) =>
    [
      testimonial.name,
      testimonial.role,
      testimonial.review,
      testimonial.gujarati?.name,
      testimonial.gujarati?.role,
      testimonial.gujarati?.review,
    ]
      .filter(Boolean)
      .join(" ")
      .toLowerCase()
  ...
  const removeTestimonial = async () => {
    ...
    try {
      await adminJsonRequest(
        `/api/admin/content/testimonials/${encodeURIComponent(pendingDelete.id)}`,
        "DELETE",
      );
      setItems((current) =>
    ...
    <div className="space-y-6">
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-col gap-3 border-b border-slate-200 px-5 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-6">
          <div>
            <h2 className="text-lg font-bold">Testimonial list</h2>
        ...
        <div className="border-b border-slate-200 px-5 py-4 sm:px-6">
          <label className="relative block max-w-xl">
            ...
            <input
              ...
              placeholder="Search customer, role, or testimonial..."
            ...
            <tbody className="divide-y divide-slate-100">
              {pageItems.length === 0 ? (
                ...
                pageItems.map((testimonial) => (
                  <tr key={testimonial.id} className="hover:bg-slate-50/70">
