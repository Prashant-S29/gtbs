...
- `AZURE_TRANSLATOR_KEY` is server-only and must never be prefixed with `NEXT_PUBLIC_`, logged, or returned. The optional `AZURE_TRANSLATOR_REGION` is configuration rather than a credential; omit it for the recommended Global resource.
...
- Migration `004_admin_translation_limits.sql` adds an RLS-enabled server-owned counter table. Atomic upserts enforce each Admin's five-minute translation window across Vercel instances without storing source/suggestion text; keys are SHA-256-derived from the Admin ID.
...
- The Gujarati suggestion route requires the same verified Admin session, origin checks, and marker header. Its strict request allows only `translate`/`transliterate`, at most 25 nonblank strings and 20,000 total characters, with transliteration further capped at 10 strings/5,000 characters; an atomic PostgreSQL window permits at most 120 requests and 100,000 characters per five minutes before returning 429/`Retry-After`, and outbound Azure calls have a 10-second timeout. Responses are no-store, provider errors are normalized, and source/suggestion content is not logged or persisted by the application. The selected source text is sent to Azure only when a suggestion is generated, so unpublished text is subject to the configured Azure service's data handling; the application itself does not log or persist request/suggestion bodies. Generated text remains untrusted editable content and passes the normal content schemas only when the Admin later saves it.
