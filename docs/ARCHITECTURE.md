...
Browser
  ...
  └─ Admin pages
       ...
       ├─ editable Gujarati suggestions → protected route → Azure Translator
       ├─ CRUD/upload APIs → PostgreSQL + UploadThing
       └─ Gujarati suggestion API → PostgreSQL limit → Azure Translator
```
...
## Admin Gujarati suggestion flow

`AdminGujaratiSuggestion` is a reusable client boundary mounted below bilingual Admin fields. On the Gujarati step it can request semantic English-to-Gujarati translation automatically when the target is blank, or the Admin can explicitly choose phonetic Roman-Gujarati transliteration and enter separate source text. The source, generated suggestion, error, and “Use suggestion” action stay local to the field. Identical completed requests are held in a bounded in-memory browser cache for the current page runtime; this is only a cost/latency optimization and is not persistent content or correctness state. A suggestion never changes the actual form field until the Admin selects “Use suggestion,” and it never replaces a nonblank target automatically.

`POST /api/admin/translate/gujarati` verifies the Better Auth Admin session and shared mutation-origin marker before parsing a strict request. It accepts only `translate` or `transliterate`, 1–25 strings, and at most 20,000 total source characters; transliteration is further restricted to Azure's 10-element/5,000-character request limit. After configuration validation, an atomic `translation_rate_limits` upsert allows at most 120 requests and 100,000 submitted characters per Admin in each five-minute database-backed window; rejection returns HTTP 429 and `Retry-After`. `src/lib/azureTranslator.ts` is server-only, sends bounded batches to Azure's v3 REST endpoints with a 10-second abort timeout, adds the subscription-region header only when configured, validates provider response shape, and maps provider failures to controlled route responses. The application persists only hashed limit keys/counters, not source text or suggestions. Missing Azure configuration yields HTTP 503 while manual Gujarati entry remains fully functional.

Plain text fields send their exact English or phonetic source. Product specifications, variant options, and features use one helper per controlled value. Blog Article translation extracts paragraph-like plain text from validated editor JSON, sends one text item, then converts the returned lines into editable paragraph nodes; rich formatting is deliberately not inferred or copied and must be reviewed in Tiptap before save.
...
- GoDaddy remains the registrar while Vercel nameservers are authoritative for DNS. The zone carries Vercel's project aliases, Resend DKIM/SPF-return-path/DMARC sending records, and Google Workspace MX/SPF records for incoming support mail. Workspace account activation and provider-side domain verification remain external boundaries.
