...
| Admin                    | `/admin/login`, `/admin/reset-password`, `/admin/dashboard`, `/api/auth/[...all]`, Product, Category, Blog, Gallery, Testimonial, and Team management routes including standalone add/edit pages | Better Auth email/password login and emailed single-use reset links; protected workspaces share one persistent responsive shell; Overview reports live repository totals and six-domain snapshots; the content modules retain their list/add/edit flows |
...
6. Run `pnpm dev` (or `npm run dev`) to use the repository's stable Webpack development fallback on Windows. `pnpm dev:turbopack` remains available for explicit Turbopack diagnostics.
...
- Contact delivery depends on Resend availability/quota. Requests are same-origin validated, bounded, honeypot screened, and PostgreSQL rate-limited; provider monitoring remains operational work.
