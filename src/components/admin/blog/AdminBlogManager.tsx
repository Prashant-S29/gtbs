  ...
  const removeBlog = async () => {
    ...
    try {
      await adminJsonRequest(
        `/api/admin/content/blogs/${encodeURIComponent(String(pendingDelete.id))}`,
        "DELETE",
      );
      setItems((current) =>
            ...
            <tbody className="divide-y divide-slate-100">
              {pageItems.length === 0 ? (
                <tr>
                  ...
                  >
                    {items.length === 0
                      ? "No blogs found. Use Add blog to create the first one."
                      : "No blogs match the current search and filter."}
                  </td>
                </tr>
              ) : (
                pageItems.map((blog) => (
